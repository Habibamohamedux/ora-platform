import React, { useState, useEffect, useRef } from 'react';
import './PageLoader.css';
function useRealProgress() {
  const [progress, setProgress] = useState(0);
  const [done, setDone]         = useState(false);
  const stateRef                = useRef({ finished: false });

  useEffect(() => {
    const startTime = Date.now();
    const MIN_MS    = 1400;

    // Clamp & move progress forward (never backward)
    const push = (target) =>
      setProgress((p) => Math.max(p, Math.min(Math.round(target), 99)));

    // ── Ambient crawl: keeps the bar moving while resources load ──
    const ticker = setInterval(() => {
      setProgress((p) => {
        if (p >= 86 || stateRef.current.finished) return p;
        const rate = p < 25 ? 1.4 : p < 55 ? 0.9 : p < 80 ? 0.45 : 0.15;
        return Math.min(p + rate, 86);
      });
    }, 110);

    // ── DOMContentLoaded → 40 % ──
    const onDCL = () => push(40);
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', onDCL, { once: true });
    } else {
      push(40);
    }

    // ── PerformanceObserver: map loaded/total resources → 40–88 % ──
    let totalRes  = 0;
    let loadedRes = 0;
    const calcFromResources = () => {
      if (!totalRes) return;
      const pct = 40 + (loadedRes / totalRes) * 48;
      push(pct);
    };

    let perfObserver;
    try {
      const existing = performance.getEntriesByType('resource');
      totalRes  = Math.max(existing.length, 1);
      loadedRes = existing.length;
      calcFromResources();

      perfObserver = new PerformanceObserver((list) => {
        const n = list.getEntries().length;
        totalRes  += n;
        loadedRes += n;
        calcFromResources();
      });
      perfObserver.observe({ type: 'resource', buffered: false });
    } catch (_) { /* unsupported */ }

    // ── Image tracking ──
    const trackImages = () => {
      const imgs = Array.from(document.images);
      if (!imgs.length) return;
      let done = 0;
      const onLoad = () => {
        done++;
        push(50 + (done / imgs.length) * 38);
      };
      imgs.forEach((img) =>
        img.complete
          ? onLoad()
          : (img.addEventListener('load',  onLoad, { once: true }),
             img.addEventListener('error', onLoad, { once: true }))
      );
    };

    if (document.readyState !== 'loading') trackImages();
    else document.addEventListener('DOMContentLoaded', trackImages, { once: true });

    // ── window.load → finish ──
    const finish = () => {
      if (stateRef.current.finished) return;
      stateRef.current.finished = true;

      clearInterval(ticker);
      if (perfObserver) perfObserver.disconnect();

      const wait = Math.max(0, MIN_MS - (Date.now() - startTime));
      setTimeout(() => {
        setProgress(100);
        setTimeout(() => setDone(true), 380);
      }, wait);
    };

    if (document.readyState === 'complete') finish();
    else window.addEventListener('load', finish, { once: true });

    // Safety net
    const safety = setTimeout(finish, 8000);

    return () => {
      clearInterval(ticker);
      clearTimeout(safety);
      if (perfObserver) perfObserver.disconnect();
      window.removeEventListener('load', finish);
      document.removeEventListener('DOMContentLoaded', onDCL);
    };
  }, []);

  return { progress, done };
}

/* ─────────────────────────────────────────────────────────────
   COMPONENT
   ───────────────────────────────────────────────────────────── */
export default function PageLoader({ children }) {
  const { progress, done } = useRealProgress();
  const [exited, setExited] = useState(false);

  useEffect(() => {
    if (!done) return;
    const t = setTimeout(() => setExited(true), 750);
    return () => clearTimeout(t);
  }, [done]);

  return (
    <>
      {!exited && (
        <div
          className={`ora-pre${done ? ' ora-pre--exit' : ''}`}
          aria-label="Loading"
          role="status"
        >
          {/* Outer soft glow — fills most of screen */}
          <div className="ora-glow-outer" />

          {/* Solid filled pink disk */}
          <div className="ora-disk" />

          {/* Concentric ring outlines */}
          <div className="ora-ring ora-ring-a" />
          <div className="ora-ring ora-ring-b" />
          <div className="ora-ring ora-ring-c" />

          {/* Everything centred over the disk */}
          <div className="ora-stage">

            {/* Logo — white SVG directly on pink */}
            <svg
              className="ora-logo"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 41 36"
              fill="none"
              aria-hidden="true"
            >
              <path
                fill="white"
                fillOpacity="0.88"
                d="M39.8337 3.57807C38.9069 1.74903 36.8947 0.650491 34.8824 0.516659C30.9563 0.257359 27.2408 3.07899 25.0344 6.17943C23.3148 8.59899 22.2649 11.4506 21.824 14.4408C21.7261 15.1053 21.169 15.6174 20.4973 15.6174C19.8255 15.6174 19.2685 15.1053 19.1705 14.4408C18.7296 11.4506 17.6797 8.59899 15.9601 6.17943C13.7538 3.07899 10.0382 0.257359 6.11213 0.516659C4.09714 0.650491 2.08763 1.74903 1.16079 3.57807C0.559301 4.76583 0.444472 6.15155 0.521026 7.48708C0.715142 10.897 2.17785 14.1703 4.43343 16.6769C5.55985 17.9288 6.8886 18.9994 8.36224 19.7996C9.96986 20.6723 11.8263 20.8842 13.3765 21.8712V21.8991H13.4229C13.4229 21.8991 13.4339 21.9047 13.4394 21.9075L13.4612 21.9242C11.9958 23.7114 10.4264 25.4066 8.92545 27.1632C7.81817 28.4597 6.16954 29.7255 5.63367 31.457C5.08139 33.2553 6.48943 34.7275 8.05329 35.2768C9.92611 35.9348 11.9247 35.0593 13.2835 33.6875C15.9886 30.9538 17.8667 27.2628 18.7806 23.3072C18.9674 22.4989 19.6699 21.8991 20.4996 21.8991C21.3296 21.8991 22.0323 22.4994 22.2189 23.3082C23.1309 27.2634 25.0092 30.954 27.7165 33.6875C29.0726 35.0593 31.0711 35.9348 32.9467 35.2768C34.5078 34.7275 35.9186 33.2553 35.3663 31.457C34.8332 29.7255 33.1818 28.4597 32.0745 27.1632C30.5736 25.4066 29.0042 23.7114 27.5388 21.9242L27.5606 21.9075C29.1218 20.887 31.0083 20.6835 32.6378 19.7968C34.1114 18.9966 35.4401 17.9288 36.5666 16.6741C38.8194 14.1675 40.2849 10.8942 40.479 7.48429C40.5555 6.14876 40.4407 4.76304 39.8392 3.57528L39.8337 3.57807ZM5.29465 10.2976C2.2462 8.39045 7.62679 5.21194 9.2508 4.71007C9.77847 4.54556 10.4046 4.59575 10.8065 4.98052C11.0525 5.21472 11.1892 5.54373 11.3123 5.86716C12.6277 9.31215 13.1798 10.8122 13.3299 13.8622C13.368 14.6361 12.531 15.1172 11.8782 14.6998C9.65806 13.2803 7.52944 11.6937 5.29465 10.2976ZM35.6971 10.2976C33.462 11.6939 31.333 13.2808 29.1125 14.7005C28.46 15.1177 27.6231 14.6371 27.6608 13.8635C27.8092 10.8126 28.3616 9.31262 29.6795 5.86716C29.8026 5.54662 29.9393 5.21751 30.1853 4.98052C30.5872 4.59854 31.2133 4.54835 31.741 4.71007C33.365 5.21194 38.7456 8.39045 35.6971 10.2976Z"
              />
            </svg>

            {/* Quote */}
            <p className="ora-quote">Better healthcare by design.</p>

            {/* Progress bar */}
            <div
              className="ora-bar-track"
              role="progressbar"
              aria-valuenow={progress}
              aria-valuemin="0"
              aria-valuemax="100"
            >
              <div className="ora-bar-fill" style={{ width: `${progress}%` }}>
                <span className="ora-bar-head" />
              </div>
            </div>

            <span className="ora-pct">{progress}<sup>%</sup></span>
          </div>
        </div>
      )}

      {/* Page — pre-rendered, fades in after overlay exits */}
      <div className={`ora-page${exited ? ' ora-page--on' : ''}`}>
        {children}
      </div>
    </>
  );
}